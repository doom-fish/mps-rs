...
| `MPSImage*` | Partial | `Image`, `ImageDescriptor`, validated byte transfers, batch helpers, and the Gaussian blur, box, Sobel, median, convolution, bilinear and Lanczos scale, threshold binary, histogram, statistics, row reduction and add filters. Canny, morphology, pyramids, integral, conversion, transpose, the remaining thresholds and arithmetic kernels, and `MPSTemporaryImage` are handles only. |
...
| `MPSCommandBuffer` | Implemented | `MpsCommandBuffer` and `Predicate`. |
